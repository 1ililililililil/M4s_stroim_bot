...
Тематика: пожарная служба, МЧС, работа пожарных, реальные выезды, техника, экипировка, обучение, физподготовка.
...
Найденную память предыдущих публикаций используй только как дополнительный контекст. Текущий пост имеет приоритет.
'''

_DEFAULT_MAX_TOKENS = 1024


def is_response_relevant(response_text: str, comment: str, post_context: str | None = None) -> bool:
    """
    Pure function to check whether a response_text is a direct, topical reaction to comment.
    Rules (summary):
    - The comment is the primary source of truth.
    - Allow paraphrases (no exact literal match required) for common topics (events/crowds).
    - Emotion-only replies are insufficient.
    - Replies that introduce professional/work topics (experience, profession, job, skills, education)
      are rejected unless those topics appear in the comment or post_context.
    """
    ...
    # helpers
    def extract_words(text: str, limit: int = 12) -> list[str]:
        if not text:
            return []
    ...
    def stem(word: str, n: int = 5) -> str:
        return word[:n]

    def contains_stem(resp: str, stems: set[str]) -> bool:
        if not stems:
            return False
        resp_words = re.findall(r"[a-zа-яё]{3,}", (resp or "").lower(), flags=re.IGNORECASE)
    ...
    comment_words = extract_words(comment or "", limit=12)
    comment_stems = {stem(w) for w in comment_words}
    context_words = extract_words(post_context or "", limit=12)
    context_stems = {stem(w) for w in context_words}

    # topic keyword buckets
    event_keywords = {"меропр", "массов", "мероприят", "толп", "толпа", "люд", "людей", "скоплен", "скопл", "событ", "концерт", "сборка"}
    ...
    # 2) If comment is clearly about events/crowds, allow paraphrases in response
    ...
    # 4) Emotion-only is insufficient (must be accompanied by topical relation)
    ...
    has_any_topic = contains_stem(response, comment_stems) or any(k in response for k in event_keywords)
    if has_emotion and not has_any_topic:
        return False

    # 5) Short emotional confirmations allowed if they don't introduce disallowed new topics
    ...
    # Default: require topical relation
    return False
...
{text}'''
    ...
    def _is_response_relevant(self, response_text: str, comment: str, post_context: str | None) -> bool:
        # Delegate to module-level pure helper
        return is_response_relevant(response_text, comment, post_context)
...
Создай ровно 3 разных варианта ответа на комментарий. Каждый вариант должен быть прямым, естественным ответом на комментарий.
...
- Если комментарий выражает сожаление, радость, вопрос, благодарность или личный опыт — ответ должен реагировать на это.
...
После составления ответов выполни внутреннюю проверку: для каждого варианта мысленно проверь — "Можно ли ответить так на этот комментарий?"
