...
def is_response_relevant(response_text: str, comment: str, post_context: str | None = None) -> bool:
    ...
    event_keywords = {"меропр", "массов", "мероприят", "толп", "толпа", "люд", "людей", "скоплен", "скопл", "событ", "концерт", "фести��аль", "сбор"}
...
Обычная реакция, благодарность или короткая похвала не треб��ют ответа. should_reply=true
...
2) Контекст исходного поста.
3) Эмоция и н��мерение автора комментария.
...
{text}'''
    ...
    async def responses(self, comment, category, knowledge=None, post_context=None):
        ...
        context = post_context.strip() if post_context else "нет"
        prompt = f'''Сначала внимательно проанализируй конкретный комментарий и выдели, о чём именно говорит польз[...]
...
Создай ровно 3 разных варианта ответа на комментарий. Каждый вариант должен быть прямым, естественным отве[...]
