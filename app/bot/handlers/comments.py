...
from app.services.openai_service import get_global_service, is_response_relevant
...
LOCAL_DISCUSSION_VARIANTS = (
    ...
    "Понимаю 😊 У каждого на этот счёт своё мнение.",
    "Согласен, такое вполне может быть 😊",
    "Да, тут действительно всё зависит от ситуации.",
    "Понимаю вас, каждому подходит что-то своё.",
        ...
        try:
            ...
            if analysis.category in SAFE and analysis.should_reply:
                ...
                if cfg.auto_reply_enabled and auto_eligible and await claim_auto_reply(s, comment_id):
                    ...
                    try:
                        # Final relevance check before auto-send (applies to all sources)
                        final_text = first.text
                        if not is_response_relevant(final_text, comment.text, comment.post_text):
                            log.info(
                                "Autopublish blocked by final relevance check comment=%s",
                                comment_id,
                            )
                        else:
                            await bot.send_message(
                    ...
                    async with Session() as publish_session:
                        # Only save as published if we actually sent
                        if is_response_relevant(first.text, comment.text, comment.post_text):
                            ...
                            # If blocked, release claim so other workers/admins can handle
