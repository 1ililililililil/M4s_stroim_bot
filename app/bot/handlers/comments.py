...
from app.database.repositories import (
    ...
    reserve_auto_reply, finalize_auto_reply, release_auto_reply_reservation,
)
from app.services.openai_service import get_global_service
...
def precheck_comment(text: str) -> AIAnalysisResult | None:
    if is_simple_reaction(text):
        return AIAnalysisResult(
            ...
            summary="Пр��стая реакция или благодарность без содержательного вопроса.",
    ...
    async with Session() as s:
        ...
        try:
                ...
                if social_result:
                    ...
                    if not faq_match or not faq_match.confident:
                        ...
                        if ai is not None:
                            ...
                            log.info("AI service not initialized, skipping AI call comment=%s", comment_id)
            ...
            if analysis.category in SAFE and analysis.should_reply:
                ...
                # Reservation-based auto-reply flow (minimal changes to existing logic)
                if cfg.auto_reply_enabled and auto_eligible:
                    first = responses[0]
                    db_started = time.perf_counter()
                    await s.commit()
                    log.info(
                    ...
                    async with Session() as reserve_s:
                        try:
                            reserved = await reserve_auto_reply(reserve_s, comment_id)
                            await reserve_s.commit()
                        except Exception:
                            await reserve_s.rollback()
                            reserved = False
                            log.exception("Reservation DB error comment=%s", comment_id)

                    if not reserved:
                        log.info("Auto-reply reservation failed (another worker?), comment=%s", comment_id)
                    else:
                        ...
                        try:
                            ...
                            async with Session() as release_s:
                                try:
                                    await release_auto_reply_reservation(release_s, comment_id)
                                    await set_error(release_s, comment_id, e)
                                    await release_s.commit()
                                except Exception:
                                    await release_s.rollback()
                                    log.exception("Failed to release reservation/comment error for comment=%s", comment_id)
                            return False
                        ...
                        async with Session() as publish_session:
                            try:
                                success = await finalize_auto_reply(publish_session, comment_id, first.text)
                                if success:
                                    ...
                                    # Critical: finalize failed after successful send
                                    log.error("CRITICAL: finalize_auto_reply failed after successful send for comment=%s", comment_id)
                                    await set_error(publish_session, comment_id, Exception("finalize_auto_reply_failed"))
                                    await publish_session.commit()
                                    return False
                            except Exception as e:
                                await publish_session.rollback()
                                log.exception("Exception during finalize_auto_reply for comment=%s", comment_id)
                                try:
                                    async with Session() as err_s:
                                        await set_error(err_s, comment_id, e)
                                        await err_s.commit()
                                except Exception:
                                    log.exception("Failed to write error state for comment=%s", comment_id)
                                return False
...
async def handle_comment(message: Message):
    ...
    if not message.from_user or message.from_user.is_bot or not text:
        log.info(
